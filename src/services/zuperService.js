...
class ZuperService {
  ...
  async makeProxiedRequest(endpoint, method = 'GET', params = null, data = null) {
    try {
      ...
      console.log('Zuper API response received:', response.data);
  ...
  async createCustomer(customerData) {
    try {
      ...
      console.log('Creating customer with data:', formattedCustomerData);
...
try {
  ...
  async createAsset(assetData) {
    try {
      ...
      console.log('Creating asset with data:', formattedAssetData);
  ...
  async getJobCategories() {
    try {
      ...
      const response = await this.makeProxiedRequest('jobs/category', 'GET');
      ...
      // Extract categories array using the proper response structure
      if (response && response.type === "success" && Array.isArray(response.data)) {
        return response.data.map(category => ({
          id: category.category_uid,
          name: category.category_name,
      ...
      console.warn('Unexpected format in job categories response:', response);
      return [];
    } catch (error) {
  ...
  async createJob(jobData) {
    try {
      ...
      const assetIds = jobData.assetIds || [];
      
      // Format for the job API - assets need to be formatted properly
      const assets = assetIds.map(assetId => ({ asset: assetId }));
      ...
      const formattedJobData = {
        job: {
          ...
          // Custom fields for diagnostic data
          custom_fields: [
            {
              label: "Diagnostic Result",
              value: String(jobData.diagnosticResult || "")
            },
            {
              label: "Repair Complexity",
              value: String(jobData.repairComplexity || "")
            }
          ]
        }
      };
      ...
      // Extract job ID - from the response format shown in the API example
